...
# rnd = np.random.default_rng(np.random.randint(1000))
...
class RRT(object):
    ...
    '''
    def __init__(self, num_samples, num_dimensions=2, step_length = 1, lims = np.array([0, 0]),
        ...
        # Setup range limits
        self.limits = lims
        if self.limits is np.array([0, 0]):
            self.limits = []
    ...
    def build_rrt(self, init, goal):
        ...
        for k in range(self.K):
            q_rand = self.sample()
            _, q_new = self.extend(self.T, q_rand)
            # states, edges = self.T.get_states_and_edges()
            if q_new != None:
                if (q_new.state == self.goal).all():
                    return self.T.get_back_path(q_new)
        print("No path found. Max iterations exceeded.")
        return None
    ...
    def build_ao_rrt(self, init, goal, T_prop, c_max, U=None):
        ...
        for k in range(self.K):
            ...
            x_rand = self.sample()
            c_rand = rnd.random() * self.c_max
            y_rand = TreeNode(x_rand, c_rand)

            t_rand = rnd.random() * T_prop
    ...
    def build_rrt_connect(self, init, goal):
        ...
        for k in range(self.K):
            q_rand = self.sample()
            _, q_new = self.extend_connect(self.T, q_rand)
            # states, _ = self.T.get_states_and_edges()
            if q_new != None:
                if (q_new.state == self.goal).all():
                    return self.T.get_back_path(q_new)
    ...
    def build_bidirectional_rrt_connect(self, init, goal):
        ...
        for k in range(self.K):
            q_rand = self.sample()
            status, q_e = self.extend_connect(self.T, q_rand)
            status, q_e_prime = self.extend_connect(self.T_goal, q_rand)

            if q_e != None and q_e_prime != None:
                if norm(q_e.state - q_e_prime.state) < self.epsilon and self.visible(q_e.state, q_e_prime.state):
                    
                    init_path = self.T.get_back_path(q_e)
                    goal_path = self.T_goal.get_back_path(q_e_prime)
                    goal_path.reverse()
    ...
    def sample(self, no_goal=False):
        ...
        if rnd.random() < self.connect_prob and not no_goal:
            return self.goal
        else:
            s = []
            for i in range(self.n):
                diff = self.limits[i, 1] - self.limits[i, 0]
                s.append(self.limits[i, 0] + rnd.random() * diff)
            s = np.array(s)
            return s
